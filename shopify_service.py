...
def get_shopify_data_for_skus(skus: list) -> pd.DataFrame:
    ...
    if df.empty:
         return pd.DataFrame(columns=['id', 'sku', 'price', 'compareAtPrice', 'inventoryQuantity', 'inventoryItemId', 'product_id', 'handle', 'title', 'tags', 'templateSuffix'])
...
def get_shopify_data_bulk(skus: list) -> pd.DataFrame:
    ...
    # 5. Filter locally by requested SKUs
    ...
    print(f"Bulk Operation complete. Filtered down to {len(filtered_df)} matching variants out of {len(df)} total variants.")
    return filtered_df
